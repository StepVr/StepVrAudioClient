...
namespace TCPStreamer
{
	public partial class FormMain : Form
	{
        ...
        //Attribute
        private TCPStreamer.ClientControll m_ClientControll;
        ...
        System.Windows.Forms.Timer m_TimerReConnect;
        ...
        //等待新命令
        private void InitClientControll()
        {
            if(m_ClientControll == null)
            {
                m_ClientControll = new TCPStreamer.ClientControll();
                m_ClientControll.mReceiveNewOrder += new TCPStreamer.ClientControll.ReceiveNewOrder(On_ReceiveNewOrder);
                m_ClientControll.StartMonitor();
            }
        }
        //接收到新命令
        private void On_ReceiveNewOrder(string NewOrder)
        {
            try
            {
        ...
        private void DelayReConnectClient(object sender, EventArgs e)
        {
            m_TimerReConnect.Stop();
            ShowMessage(LabelClient, "ReConnectClient");
            ClientConnectToServer();
        }
        private void ReConnectClient()
        {
            try
            {
                ...
                //延迟重新连接
                if (m_TimerReConnect == null)
                {
                    m_TimerReConnect = new System.Windows.Forms.Timer();
                    m_TimerReConnect.Interval = 2000;                                   // ** 提示框/拍照间隔时间 对应的毫秒
                    m_TimerReConnect.Tick += new EventHandler(DelayReConnectClient);    // ** 到达指定延迟时间的回调函数
                }
